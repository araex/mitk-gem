...
void SurfaceToUnstructuredGridFilter::GenerateData() {
    auto surface = GetInput()->GetVtkPolyData();
    auto mesh = GetOutput()->GetVtkUnstructuredGrid();
    tetgenMesh(surface, mesh);
}

void SurfaceToUnstructuredGridFilter::tetgenMesh(vtkSmartPointer<vtkPolyData> surface, vtkSmartPointer<vtkUnstructuredGrid> mesh) {
