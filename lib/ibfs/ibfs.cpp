...
void IBFSGraph::initSize(int numNodes, int numEdges)
{
	// allocate nodes
	if (verbose) {
		fprintf(stdout, "c allocating nodes... \t [%lu]\n", (unsigned long)sizeof(Node)*(unsigned long)(numNodes+1));
		fflush(stdout);
	}
	...
	if (verbose) {
		fprintf(stdout, "c allocating arcs... \t [%lu]\n", (unsigned long)arcMemsize);
		fflush(stdout);
	}
	...
	if (verbose) {
		...
		fprintf(stdout, "c #nodes = %d \n", nodeEnd-nodes);
		fprintf(stdout, "c #arcs = %d \n", (arcEnd-arcs) + (nodeEnd-nodes));
		fprintf(stdout, "c #grid_arcs = %d \n", arcEnd-arcs);
	}
}
...
template<bool sTree> void IBFSGraph::augmentTree(Node *x, int bottleneck)
{
	Node *y;
	Arc *a;
...
void IBFSGraph::augment(Arc *bridge)
{
	...
	// stats
	if (IBSTATS) {
		int augLen = stats.getPushes() - pushesBefore;
...
template <bool sTree> void IBFSGraph::adoption3Pass()
{
	Arc *a, *aEnd;
	Node *x, *y;
